...
**Correction note:** This register has been adjudicated by Rio against the wider LOUMIES governance record. Rows A3, A4, A5, A6, B4, C1, D1, F1, G4, and I1 are corrected from the original V0 below; other rows are carried forward unchanged except where the orthogonal state-model correction (see infrastructure doc §9) affects their phrasing.
...
| **A4** | Conditions that make the third day "sufficiently firm" to activate | **OPEN — no numerical threshold implied or invented.** This is corrected from the prior version's framing, which risked implying a required numerical trigger. | Determines when/how the operator opens the conditional day (§13) | Rania's judgment call, informed by demand signals as the proof runs — not a formula to be derived here | Activation |
...
| # | Issue | Status | Why it matters | What would close it | Blocks |
|---|---|---|---|---|---|
| **D1** | Total same-day order capacity per operating date | **WORKING ASSUMPTION / CONFIGURABLE — the assignment's "5 orders" is one test scenario only, never a permanent rule.** Same-day ordering itself remains an experimental/configurable hypothesis (infrastructure doc §2), not an owner-locked channel. | §5 requires the system to test 0, 5, 10, or any value without redesign | Rania's test decisions across proof weeks | Activation |
...
- **Blocks Activation of the proof / a given channel:** A3b, A4, B2, B4 (safe sellable number only, not full physical study), C1, C3, C5, D1, D2, D3, E1–E4, F1–F3, G1–G3
...
- **Closed by this correction pass:** A3a (day-count architecture), A5 (founder-hour and kitchen-hour envelope), and the portion of I1 covering those items plus A6's reopening and F1's per-counterparty framing.
